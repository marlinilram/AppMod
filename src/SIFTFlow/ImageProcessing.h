...
public:
	...
	template <class T1,class T2>
	static void ResizeImage(const T1* pSrcImage,T2* pDstImage,int SrcWidth,int SrcHeight,int nChannels,double Ratio, int interpolation = 1);

	template <class T1,class T2>
	static void ResizeImage(const T1* pSrcImage,T2* pDstImage,int SrcWidth,int SrcHeight,int nChannels,int DstWidth,int DstHeight, int interpolation = 1);
...
inline void ImageProcessing::BilinearInterpolate(const T1* pImage,int width,int height,int nChannels,double x,double y,T2* result)
{
	...
	float *tresult  = new float[nChannels];
	...
	for(m=0;m<=1;m++)
		for(n=0;n<=1;n++)
		{
			...
			for(l=0;l<nChannels;l++)
			{
				tresult[l] +=pImage[offset+l]*s;
				
			}
		}
	for(l=0;l<nChannels;l++)
		result[l] = (int)tresult[l] ;
}
...
//------------------------------------------------------------------------------------------------------------
template <class T1,class T2>
void ImageProcessing::ResizeImage(const T1* pSrcImage,T2* pDstImage,int SrcWidth,int SrcHeight,int nChannels,double Ratio, int interpolation)
{
	...
	for(int i=0;i<DstHeight;i++)
		...
		/*	else if(interpolation==0)
			{
				T2* result = pDstImage+(i*DstWidth+j)*nChannels;
				memset(result,0,sizeof(T2)*nChannels);
				for(int k=0;k<nChannels;k++)
				{
					int offset=(y*SrcWidth+x)*nChannels;
					result[k] = pSrcImage[offset+k];
				}
			}*/
	...
	for(int i=0;i<DstHeight;i++)
		for(int j=0;j<DstWidth;j++)
		{
			...
			/*else if(interpolation==0)
			{
				...
				for(int k=0;k<nChannels;k++)
				{
					int offset=(y*SrcWidth+x)*SrcHeight;
					result[k] = pSrcImage[offset*nChannels+k];
				}
			}*/
		}
	sum +=0;
