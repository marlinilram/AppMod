...
public:
	...
	virtual bool imresize(double ratio, int interpolation=1 );
	template <class T1>
	void imresize(Image<T1>& result,double ratio, int interpolation=1) const;
	void imresize(int dstWidth,int dstHeight, int interpolation=1);
	template <class T1>
	void imresize(Image<T1>& result,int dstWidth,int dstHeight, int interpolation=1) const;
...
//------------------------------------------------------------------------------------------
template <class T>
bool Image<T>::imresize(double ratio,int interpolation)
{
	if(pData==NULL)
		return false;
	...
	ImageProcessing::ResizeImage(pData,pDstData,imWidth,imHeight,nChannels,ratio,interpolation);
...
template <class T>
template <class T1>
void Image<T>::imresize(Image<T1>& result,double ratio,int interpolation) const
{
	int DstWidth,DstHeight;
	DstWidth=  (double)imWidth*ratio;//floor((double)imWidth*ratio+0.5);
	DstHeight= (double)imHeight*ratio;//floor((double)imHeight*ratio+0.5);
	/*DstWidth=  floor((double)imWidth*ratio+0.5);
	DstHeight= floor((double)imHeight*ratio+0.5);*/
	if(result.width()!=DstWidth || result.height()!=DstHeight || result.nchannels()!=nChannels)
		result.allocate(DstWidth,DstHeight,nChannels);
	ImageProcessing::ResizeImage(pData,result.data(),imWidth,imHeight,nChannels,ratio,interpolation);
}

template <class T>
template <class T1>
void Image<T>::imresize(Image<T1>& result,int DstWidth,int DstHeight,int interpolation) const
{
	if(result.width()!=DstWidth || result.height()!=DstHeight || result.nchannels()!=nChannels)
		result.allocate(DstWidth,DstHeight,nChannels);
	ImageProcessing::ResizeImage(pData,result.data(),imWidth,imHeight,nChannels,DstWidth,DstHeight,interpolation);
}


template <class T>
void Image<T>::imresize(int dstWidth,int dstHeight,int interpolation)
{
	DImage foo(dstWidth,dstHeight,nChannels);
	ImageProcessing::ResizeImage(pData,foo.data(),imWidth,imHeight,nChannels,dstWidth,dstHeight,interpolation);
	copyData(foo);
}
