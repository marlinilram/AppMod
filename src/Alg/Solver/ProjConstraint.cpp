  ...
  float cam_ori[3])
{
  ...
  for (decltype(ray_id_list.size()) i = 0; i < ray_id_list.size(); ++i)
  {
    ...
    proj_triplets.push_back(Triplet(3 * v_id + 0, 3 * v_id + 0, 1 - ray[0] * ray[0]));
    proj_triplets.push_back(Triplet(3 * v_id + 0, 3 * v_id + 1, -ray[0] * ray[1]));
    proj_triplets.push_back(Triplet(3 * v_id + 0, 3 * v_id + 2, -ray[0] * ray[2]));
    right_hand(3 * v_id + 0) = (1 - ray[0] * ray[0]) * cam_ori[0]
                             - ray[0] * ray[1] * cam_ori[1]
                             - ray[0] * ray[2] * cam_ori[2];

    proj_triplets.push_back(Triplet(3 * v_id + 1, 3 * v_id + 0, -ray[1] * ray[0]));
    proj_triplets.push_back(Triplet(3 * v_id + 1, 3 * v_id + 1, 1 - ray[1] * ray[1]));
    proj_triplets.push_back(Triplet(3 * v_id + 1, 3 * v_id + 2, -ray[1] * ray[2]));
    right_hand(3 * v_id + 1) = -ray[1] * ray[0] * cam_ori[0]
                             + (1 - ray[1] * ray[1]) * cam_ori[1]
                             - ray[1] * ray[2] * cam_ori[2];

    proj_triplets.push_back(Triplet(3 * v_id + 2, 3 * v_id + 0, -ray[2] * ray[0]));
    proj_triplets.push_back(Triplet(3 * v_id + 2, 3 * v_id + 1, -ray[2] * ray[1]));
    proj_triplets.push_back(Triplet(3 * v_id + 2, 3 * v_id + 2, 1 - ray[2] * ray[2]));
    right_hand(3 * v_id + 2) = -ray[2] * ray[0] * cam_ori[0]
...
{
  right_hand = this->lamd_proj * this->right_hand;
}
