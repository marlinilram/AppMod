...
void GeometryPartAlg::updateGeometry(Coarse *model)
{
    ...
    NormalGuided* normal_guided = new NormalGuided;
    ...
    fsm->buildMatrix();
    fsm->setkStrech(m_para->lambd_k_strech); //10.0f;
    fsm->setkBending(m_para->lambd_k_bend); //15.0f;

    // init normal guided constraint
    normal_guided->initMatrix(
      (*face_list),
      (*vertex_list),
      (*vertices_share_faces),
      (*normal_list),
      (*new_normals));
    normal_guided->setLamdNormal(m_para->lambd_deform_normal);
    normal_guided->setLamdVMove(m_para->lambd_vertical_move);

    // add constraints to solver
    solver->addConstraint(fsm);
    solver->addConstraint(normal_guided);
