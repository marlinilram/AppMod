...
BoundingBox::BoundingBox( const Vector3f& c, double x, double y, double z )
{
	this->center = c;
...
BoundingBox::BoundingBox( const Vector3f& fromMin, const Vector3f& toMax )
{
	vmin = fromMin;
	vmax = toMax;
...
std::vector<Vector3f> BoundingBox::getCorners()
{
    std::vector<Vector3f> corners;

	Vector3f x = (Vector3f(1,0,0) * xExtent);
	Vector3f y = (Vector3f(0,1,0) * yExtent);
	Vector3f z = (Vector3f(0,0,1) * zExtent);

	Vector3f c = center + x + y + z;
...
bool BoundingBox::containsTriangle( const Vector3f& tv0, const Vector3f& tv1, const Vector3f& tv2 ) const
{
	Vector3f boxcenter(center);
	Vector3f boxhalfsize(xExtent, yExtent, zExtent);
...
bool BoundingBox::intersectsSphere( const Vector3f& sphere_center, double radius )
{
...
{
	Offset( Vector3f(s,s,s));
}

void BoundingBox::Offset( Vector3f delta )
{
