...
bool WriteObj(const std::string& filename, const std::vector<tinyobj::shape_t>& shapes, const std::vector<tinyobj::material_t>& materials) {
  ...
  for (size_t i = 0; i < shapes.size(); i++) {
    ...
    // !!!Notice: for current dev, I changed it into per vertex texcoord!!!
  ...
  // Write material file
  //
  bool ret = true;//WriteMat(material_filename, materials);
